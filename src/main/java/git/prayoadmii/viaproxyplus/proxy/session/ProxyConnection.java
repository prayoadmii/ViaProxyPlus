 ...
 */
package git.prayoadmii.viaproxyplus.proxy.session;
...
import git.prayoadmii.viaproxyplus.cli.ConsoleFormatter;
import git.prayoadmii.viaproxyplus.proxy.packethandler.PacketHandler;
...
public class ProxyConnection extends NetClient {
    ...
    public void kickClient(final String message) throws CloseAndReturn {
        ...
        if (this.c2pConnectionState == ConnectionState.STATUS) {
            future = this.c2p.writeAndFlush(new S2CStatusResponsePacket("{\"players\":{\"max\":0,\"online\":0},\"description\":" + new JsonPrimitive(message) + ",\"version\":{\"protocol\":-1,\"name\":\"ViaProxyPlus\"}}"));
